...
class GraphDiskQueueCPUMaker:
    ...
    def make_graph(self):
        ...
        ax.set_title("Очередь дисковой подсистемы", fontsize=16)
        ax.set_ylabel('Очередь диска, шт', fontsize=10)
        ax.set_xlabel('Продолжительность теста, ч:мм', fontsize=10)

        fig.tight_layout()

        # Строим график
        plt.plot(self.dates, self.graph_data_1, label='Очередь дисковой подсистемы')
        # Устанавливаем сетку
        plt.grid()
